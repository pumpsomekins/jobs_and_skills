...
<title>Job Automation Compatibility List – Chef</title>
...
:root{
  --bg:#0e0e0e;--surface:#161616;--surface-2:#1c1c1c;--surface-3:#222;
  --border:#2a2a2a;--border-hi:#363636;
  --text:#e2e2e2;--text-muted:#888;--text-faint:#444;
  --green:#22c55e;--yellow:#eab308;--orange:#f97316;--red:#ef4444;
  --accent:#4f98a3;--accent-hover:#60b8c4;
...
body{background:var(--bg);color:var(--text);font-family:var(--font-body);font-size:15px;line-height:1.6;min-height:100dvh}
button{cursor:pointer;border:none;background:none;font:inherit}
body::before{content:'';position:fixed;inset:0;background:repeating-linear-gradient(0deg,transparent,transparent 2px,rgba(0,0,0,.05) 2px,rgba(0,0,0,.05) 4px);pointer-events:none;z-index:0}

/* Layout */
.page{position:relative;z-index:1;max-width:1200px;margin:0 auto;padding:36px 24px 80px}
@media(max-width:768px){.page{padding:20px 14px 60px}}

/* ── Industry Tags ── */
.industry-row{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:14px}
.industry-pill{display:inline-flex;align-items:center;gap:5px;padding:4px 11px;border-radius:20px;font-size:11px;font-weight:600;letter-spacing:.04em;border:1px solid var(--border-hi);color:var(--text-faint);cursor:pointer;transition:all .18s var(--ease);white-space:nowrap}
.industry-pill:hover{color:var(--text-muted);border-color:var(--text-faint)}
.industry-pill.active{color:#fff;border-color:transparent;background:var(--surface-3)}
.industry-pill .ind-dot{width:5px;height:5px;border-radius:50%;background:currentColor;flex-shrink:0;opacity:.7}

/* ── Header ── */
.header{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:28px;padding-bottom:22px;border-bottom:1px solid var(--border)}
.header-tag{display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--accent);background:color-mix(in oklch,var(--accent) 12%,transparent);border:1px solid color-mix(in oklch,var(--accent) 30%,transparent);padding:4px 10px;border-radius:4px;margin-bottom:10px}
.header-tag .live-dot{width:6px;height:6px;border-radius:50%;background:var(--accent);animation:pulse 2s ease-in-out infinite}
@keyframes pulse{0%,100%{opacity:1}50%{opacity:.35}}
.page-title{font-family:var(--font-display);font-size:clamp(22px,4vw,32px);font-weight:800;color:#fff;line-height:1.1;margin-bottom:6px;letter-spacing:-.01em}
.page-subtitle{font-size:12px;color:var(--text-muted)}
.page-subtitle span{color:var(--accent);font-size:11px;font-family:monospace}
.header-right{display:flex;align-items:center;gap:10px;flex-shrink:0;padding-top:2px}
.btn-subscribe{display:inline-flex;align-items:center;gap:7px;padding:9px 18px;border-radius:var(--radius-md);font-size:13px;font-weight:700;letter-spacing:.02em;background:var(--accent);color:#fff;transition:background .18s var(--ease),transform .15s,box-shadow .18s var(--ease)}
.btn-subscribe:hover{background:var(--accent-hover);transform:translateY(-1px);box-shadow:0 6px 24px color-mix(in oklch,var(--accent) 35%,transparent)}
.btn-subscribe svg{width:14px;height:14px}

/* ── Country Tabs ── */
.country-section{margin-bottom:20px}
.country-label{font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--text-faint);margin-bottom:10px}
.country-tabs{display:flex;gap:6px;flex-wrap:wrap}
.ctab{display:inline-flex;align-items:center;gap:6px;padding:7px 13px;border-radius:var(--radius-md);border:1px solid var(--border-hi);font-size:12px;font-weight:600;color:var(--text-muted);cursor:pointer;transition:all .18s var(--ease);white-space:nowrap;position:relative}
.ctab:hover{border-color:var(--text-faint);color:var(--text)}
.ctab.active{background:var(--surface-2);border-color:var(--accent);color:#fff}
.ctab.active::after{content:'';position:absolute;bottom:-7px;left:50%;transform:translateX(-50%);width:4px;height:4px;border-radius:50%;background:var(--accent)}
.ctab .flag{font-size:14px;line-height:1}
.ctab .cname{font-size:11px}
.ctab .crisk{font-size:10px;padding:1px 5px;border-radius:3px;font-weight:700;margin-left:2px}
.crisk.fast{background:color-mix(in oklch,var(--red) 18%,transparent);color:var(--red);border:1px solid color-mix(in oklch,var(--red) 30%,transparent)}
.crisk.mid{background:color-mix(in oklch,var(--yellow) 14%,transparent);color:var(--yellow);border:1px solid color-mix(in oklch,var(--yellow) 25%,transparent)}
.crisk.slow{background:color-mix(in oklch,var(--green) 14%,transparent);color:var(--green);border:1px solid color-mix(in oklch,var(--green) 25%,transparent)}

/* ── LTV Section ── */
.ltv-section{margin-bottom:28px}
.section-label{font-size:11px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--text-faint);margin-bottom:14px}
.ltv-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:10px;margin-bottom:12px}
@media(max-width:1000px){.ltv-grid{grid-template-columns:repeat(3,1fr)}}
@media(max-width:600px){.ltv-grid{grid-template-columns:repeat(2,1fr)}}

.ltv-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:16px 18px 14px;position:relative;overflow:hidden;transition:border-color .2s,box-shadow .2s var(--ease)}
.ltv-card:hover{border-color:var(--border-hi);box-shadow:0 8px 28px rgba(0,0,0,.35)}
.ltv-card::after{content:'';position:absolute;inset:0;background:radial-gradient(circle at 50% 0%,var(--glow,transparent) 0%,transparent 70%);pointer-events:none;opacity:.5}
.ltv-card.c-total{--glow:color-mix(in oklch,var(--accent) 15%,transparent)}
.ltv-card.c-safe{--glow:color-mix(in oklch,var(--green) 12%,transparent)}
.ltv-card.c-risk{--glow:color-mix(in oklch,var(--red) 12%,transparent)}
.ltv-card.c-idx{--glow:color-mix(in oklch,var(--yellow) 12%,transparent)}
.ltv-card.c-age{--glow:color-mix(in oklch,var(--orange) 12%,transparent)}
.ltv-lbl{font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--text-muted);margin-bottom:8px}
.ltv-val{font-family:var(--font-display);font-size:clamp(20px,2.5vw,28px);font-weight:800;letter-spacing:-.02em;line-height:1;margin-bottom:4px}
.cl-total{color:#fff}.cl-safe{color:var(--green)}.cl-risk{color:var(--red)}.cl-idx{color:var(--yellow)}.cl-age{color:var(--orange)}
.ltv-meta{font-size:10px;color:var(--text-faint);line-height:1.4}
.ltv-bar{height:3px;background:var(--border);border-radius:2px;margin-top:10px;overflow:hidden}
.ltv-bar-fill{height:100%;border-radius:2px;transition:width 1.4s var(--ease)}

/* age timeline */
.age-timeline{display:flex;align-items:center;gap:6px;margin-top:8px}
.age-seg{height:4px;border-radius:2px;transition:width .8s var(--ease)}
.age-seg.safe-seg{background:var(--green);flex:1}
.age-seg.risk-seg{background:var(--orange);flex-shrink:0}
.age-pins{display:flex;justify-content:space-between;margin-top:3px}
.age-pin{font-size:9px;color:var(--text-faint)}
.age-pin.risk-pin{color:var(--orange);font-weight:700}

/* ── LTV Breakdown bar ── */
.ltv-chart-wrap{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:18px 22px;margin-bottom:12px}
.ltv-chart-title{font-size:11px;font-weight:600;color:var(--text-muted);letter-spacing:.06em;text-transform:uppercase;margin-bottom:12px}
.ltv-breakdown{display:flex;gap:0;height:24px;border-radius:4px;overflow:hidden}
.ltv-seg{display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;letter-spacing:.04em;transition:width 1.2s var(--ease);white-space:nowrap;overflow:hidden}
.ltv-seg.seg-safe{background:color-mix(in oklch,var(--green) 28%,transparent);border:1px solid var(--green);color:var(--green)}
.ltv-seg.seg-risk{background:color-mix(in oklch,var(--red) 18%,transparent);border:1px solid var(--red);border-left:none;color:var(--red)}
.ltv-legend{display:flex;gap:16px;margin-top:8px;flex-wrap:wrap}
.ltv-legend-item{display:flex;align-items:center;gap:5px;font-size:10px;color:var(--text-muted)}
.ltv-legend-dot{width:8px;height:8px;border-radius:2px;flex-shrink:0}

/* ── Car Visualization ── */
.car-viz{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:20px 22px}
.car-viz-header{display:flex;align-items:baseline;gap:10px;margin-bottom:4px;flex-wrap:wrap}
.car-viz-eq{font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--text-faint)}
.car-viz-val{font-family:var(--font-display);font-size:clamp(18px,3vw,26px);font-weight:800;color:#fff;transition:all .3s var(--ease)}
.car-viz-val span{color:var(--accent)}
.car-viz-sub{font-size:11px;color:var(--text-faint);margin-bottom:16px}
.car-grid{display:flex;flex-wrap:wrap;gap:5px;margin-bottom:10px;min-height:60px}
.car-icon{width:38px;height:22px;flex-shrink:0;transition:opacity .3s var(--ease),transform .3s var(--ease)}
.car-icon.safe-car{color:var(--green);opacity:.85}
.car-icon.risk-car{color:var(--red);opacity:.55}
.car-legend{display:flex;gap:16px;flex-wrap:wrap}
.car-legend-item{display:flex;align-items:center;gap:6px;font-size:10px;color:var(--text-muted)}
.car-legend-swatch{width:24px;height:10px;border-radius:2px}
.car-legend-swatch.safe{background:color-mix(in oklch,var(--green) 35%,transparent);border:1px solid var(--green)}
.car-legend-swatch.risk{background:color-mix(in oklch,var(--red) 25%,transparent);border:1px solid var(--red)}

/* ── Filters ── */
.filters-row{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:14px;flex-wrap:wrap}
.filters-left{display:flex;align-items:center;gap:7px;flex-wrap:wrap}
.filter-pill{display:inline-flex;align-items:center;gap:5px;padding:5px 11px;border-radius:4px;border:1px solid var(--border-hi);font-size:11px;font-weight:600;letter-spacing:.04em;color:var(--text-muted);cursor:pointer;transition:all .15s var(--ease)}
.filter-pill.active,.filter-pill:hover{color:#fff;border-color:transparent}
.filter-pill.all.active{background:#fff2;color:#fff}
.filter-pill.gf.active{background:color-mix(in oklch,var(--green) 22%,transparent);border-color:var(--green);color:var(--green)}
.filter-pill.yf.active{background:color-mix(in oklch,var(--yellow) 18%,transparent);border-color:var(--yellow);color:var(--yellow)}
.filter-pill.of.active{background:color-mix(in oklch,var(--orange) 18%,transparent);border-color:var(--orange);color:var(--orange)}
.filter-pill.rf.active{background:color-mix(in oklch,var(--red) 16%,transparent);border-color:var(--red);color:var(--red)}
...
th{padding:11px 16px;font-size:10px;font-weight:700;letter-spacing:.09em;text-transform:uppercase;color:var(--text-faint);border-bottom:1px solid var(--border);white-space:nowrap}
...
.col-action{font-weight:600;font-size:14px;color:#fff;white-space:nowrap}
.col-human,.col-ai{font-size:12px;max-width:200px}
.col-human{color:var(--text-muted)}
.col-ai{color:#7fb3d3}
.bar-wrap{display:flex;flex-direction:column;gap:4px;min-width:130px}
.bar-track{width:100%;background:var(--border);border-radius:2px;height:5px;overflow:hidden}
.bar-fill{height:100%;border-radius:2px;transition:width 1.2s var(--ease)}
.bar-lbl{font-size:10px;color:var(--text-muted);display:flex;justify-content:space-between}
.bar-lbl span:last-child{font-weight:700;color:var(--text)}
.ltv-chip{display:inline-block;font-size:10px;font-weight:700;padding:3px 7px;border-radius:3px;letter-spacing:.03em;white-space:nowrap}
.ltv-chip.safe{background:color-mix(in oklch,var(--green) 14%,transparent);color:var(--green);border:1px solid color-mix(in oklch,var(--green) 28%,transparent)}
.ltv-chip.partial{background:color-mix(in oklch,var(--yellow) 12%,transparent);color:var(--yellow);border:1px solid color-mix(in oklch,var(--yellow) 22%,transparent)}
.ltv-chip.risk{background:color-mix(in oklch,var(--red) 12%,transparent);color:var(--red);border:1px solid color-mix(in oklch,var(--red) 22%,transparent)}
.eta-badge{font-size:11px;color:var(--text-muted);white-space:nowrap}
.eta-soon{color:var(--orange)}.eta-mid{color:var(--yellow)}.eta-far{color:var(--green)}.eta-prot{color:var(--accent)}
.status-badge{display:inline-flex;align-items:center;gap:4px;padding:3px 9px;border-radius:3px;font-size:10px;font-weight:700;letter-spacing:.07em;text-transform:uppercase;white-space:nowrap}
.status-badge .dot{width:5px;height:5px;border-radius:50%;background:currentColor;flex-shrink:0}
.b-play{background:color-mix(in oklch,var(--green) 16%,transparent);color:var(--green);border:1px solid color-mix(in oklch,var(--green) 30%,transparent)}
.b-ing{background:color-mix(in oklch,var(--yellow) 13%,transparent);color:var(--yellow);border:1px solid color-mix(in oklch,var(--yellow) 25%,transparent)}
.b-intro{background:color-mix(in oklch,var(--orange) 14%,transparent);color:var(--orange);border:1px solid color-mix(in oklch,var(--orange) 28%,transparent)}
.b-none{background:color-mix(in oklch,var(--red) 13%,transparent);color:var(--red);border:1px solid color-mix(in oklch,var(--red) 25%,transparent)}

/* Summary */
.summary-row{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:12px}
@media(max-width:640px){.summary-row{grid-template-columns:1fr}}
.summary-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-md);padding:14px 16px;font-size:12px;color:var(--text-muted)}
.summary-card strong{color:var(--text);display:block;font-size:14px;font-weight:700;margin-bottom:3px}
.footnote{margin-top:12px;font-size:10px;color:var(--text-faint);text-align:right}

/* ── Modal ── */
.modal-overlay{position:fixed;inset:0;background:rgba(0,0,0,.75);backdrop-filter:blur(6px);z-index:100;display:flex;align-items:center;justify-content:center;padding:20px;opacity:0;pointer-events:none;transition:opacity .25s var(--ease)}
.modal-overlay.open{opacity:1;pointer-events:all}
.modal{background:var(--surface-2);border:1px solid var(--border-hi);border-radius:var(--radius-lg);padding:30px;max-width:420px;width:100%;transform:translateY(14px) scale(.97);transition:transform .28s var(--ease);position:relative}
.modal-overlay.open .modal{transform:none}
.modal-close{position:absolute;top:12px;right:12px;width:28px;height:28px;border-radius:4px;display:flex;align-items:center;justify-content:center;color:var(--text-muted);transition:background .15s,color .15s}
.modal-close:hover{background:var(--border);color:var(--text)}
.modal-icon{width:42px;height:42px;background:color-mix(in oklch,var(--accent) 16%,transparent);border:1px solid color-mix(in oklch,var(--accent) 32%,transparent);border-radius:var(--radius-md);display:flex;align-items:center;justify-content:center;color:var(--accent);margin-bottom:14px}
.modal-title{font-family:var(--font-display);font-size:19px;font-weight:800;color:#fff;margin-bottom:5px;letter-spacing:-.01em}
.modal-desc{font-size:12px;color:var(--text-muted);margin-bottom:22px;line-height:1.6}
.modal-desc strong{color:var(--text)}
.input-group{display:flex;gap:7px;margin-bottom:12px}
.modal-input{flex:1;background:var(--surface);border:1px solid var(--border-hi);border-radius:var(--radius-md);padding:9px 13px;color:var(--text);font:inherit;font-size:13px;outline:none;transition:border-color .18s}
.modal-input::placeholder{color:var(--text-faint)}
.modal-input:focus{border-color:var(--accent)}
.btn-confirm{display:inline-flex;align-items:center;gap:5px;padding:9px 16px;border-radius:var(--radius-md);background:var(--accent);color:#fff;font:inherit;font-size:13px;font-weight:700;cursor:pointer;border:none;white-space:nowrap;transition:background .18s var(--ease),transform .15s}
.btn-confirm:hover{background:var(--accent-hover);transform:translateY(-1px)}
.modal-tags{display:flex;gap:6px;flex-wrap:wrap;margin-top:4px}
.modal-tag{display:inline-flex;align-items:center;gap:4px;padding:5px 9px;border:1px solid var(--border-hi);border-radius:4px;font-size:11px;color:var(--text-muted);cursor:pointer;transition:all .15s}
.modal-tag:hover,.modal-tag.sel{border-color:var(--accent);color:var(--accent);background:color-mix(in oklch,var(--accent) 10%,transparent)}
.modal-success{text-align:center;padding:14px 0;display:none}
.modal-success .check{width:48px;height:48px;border-radius:50%;background:color-mix(in oklch,var(--green) 16%,transparent);border:1px solid var(--green);display:flex;align-items:center;justify-content:center;color:var(--green);margin:0 auto 12px;font-size:22px}
.modal-success h3{color:#fff;font-size:16px;font-weight:700;margin-bottom:5px}
.modal-success p{font-size:12px;color:var(--text-muted)}

/* Toast */
.toast{position:fixed;bottom:22px;right:22px;background:var(--surface-2);border:1px solid var(--border-hi);border-radius:var(--radius-md);padding:11px 15px;font-size:12px;color:var(--text);display:flex;align-items:center;gap:9px;z-index:200;box-shadow:0 8px 32px rgba(0,0,0,.5);transform:translateY(18px);opacity:0;transition:all .28s var(--ease)}
.toast.show{transform:none;opacity:1}
.toast-dot{width:7px;height:7px;border-radius:50%;background:var(--green);flex-shrink:0;animation:pulse 2s infinite}

/* Fade for LTV updates */
.ltv-fade{animation:fadeIn .4s var(--ease)}
@keyframes fadeIn{from{opacity:.2;transform:translateY(4px)}to{opacity:1;transform:none}}

/* ── Level Tabs ── */
.level-section{margin-bottom:20px}
.level-tabs{display:flex;align-items:center;gap:0;flex-wrap:wrap}
.ltab{position:relative;display:inline-flex;flex-direction:column;align-items:center;gap:3px;padding:9px 16px;border:1px solid var(--border-hi);font-size:12px;font-weight:700;color:var(--text-muted);cursor:pointer;transition:all .18s var(--ease);background:var(--surface);white-space:nowrap;margin-right:-1px}
.ltab:first-child{border-radius:var(--radius-md) 0 0 var(--radius-md)}
.ltab:last-child{border-radius:0 var(--radius-md) var(--radius-md) 0;margin-right:0}
.ltab:hover{color:var(--text);background:var(--surface-2);z-index:1}
.ltab.active{background:var(--surface-2);border-color:var(--accent);color:#fff;z-index:2}
.ltab .lyears{font-size:9px;font-weight:500;color:var(--text-faint);letter-spacing:.04em}
.ltab.active .lyears{color:var(--accent)}
.ltab .lpct{font-size:9px;font-weight:700;color:var(--accent);letter-spacing:.03em;background:color-mix(in oklch,var(--accent) 12%,transparent);border:1px solid color-mix(in oklch,var(--accent) 25%,transparent);padding:1px 5px;border-radius:3px;margin-top:1px}
.ltab.active .lpct{background:color-mix(in oklch,var(--accent) 20%,transparent)}
.ltab .lcolor-dot{width:6px;height:6px;border-radius:50%;background:var(--ldot,var(--text-faint));margin-bottom:2px}

/* ── Income Curve Chart ── */
.income-chart-wrap{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:20px 22px;margin-bottom:12px}
.income-chart-header{display:flex;align-items:baseline;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:8px}
.income-chart-title{font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--text-faint)}
.income-chart-note{font-size:10px;color:var(--text-faint)}
#incomeSVG{width:100%;overflow:visible;display:block}
.level-legend{display:flex;gap:14px;flex-wrap:wrap;margin-top:10px}
.level-legend-item{display:flex;align-items:center;gap:5px;font-size:10px;color:var(--text-muted)}
.level-legend-dot{width:8px;height:8px;border-radius:50%;flex-shrink:0}

/* ── Promotion Ladder ── */
.promo-section{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:18px 22px;margin-bottom:12px}
.promo-flow{display:flex;align-items:center;gap:0;overflow-x:auto;padding:4px 0}
.promo-node{display:flex;flex-direction:column;align-items:center;min-width:100px;gap:5px}
.promo-circle{width:46px;height:46px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:800;letter-spacing:.02em;text-align:center;border:2px solid;line-height:1.2}
.promo-label{font-size:10px;font-weight:600;color:var(--text-muted);text-align:center;max-width:90px;line-height:1.3}
.promo-salary{font-size:10px;color:var(--text-faint);text-align:center;font-weight:700}
.promo-arrow{display:flex;flex-direction:column;align-items:center;gap:2px;padding:0 4px;min-width:52px}
.promo-arrow-line{width:100%;height:1px;background:var(--border-hi);position:relative}
.promo-arrow-line::after{content:'▶';position:absolute;right:-5px;top:-5px;font-size:10px;color:var(--text-faint)}
.promo-arrow-years{font-size:9px;color:var(--accent);font-weight:700;white-space:nowrap;text-align:center}
.promo-arrow-sub{font-size:8px;color:var(--text-faint);white-space:nowrap}
...
.insight-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:16px 18px;position:relative;overflow:hidden;transition:border-color .2s}
.insight-card:hover{border-color:var(--border-hi)}
.insight-icon{font-size:22px;margin-bottom:8px;display:block}
.insight-label{font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--text-muted);margin-bottom:4px}
.insight-value{font-family:var(--font-display);font-size:clamp(16px,2vw,22px);font-weight:800;color:#fff;line-height:1;margin-bottom:4px}
.insight-meta{font-size:10px;color:var(--text-faint);line-height:1.4}
.difficulty-bar{display:flex;gap:3px;margin-top:8px}
.diff-pip{width:14px;height:6px;border-radius:2px;background:var(--border)}
.diff-pip.filled-low{background:var(--green)}
.diff-pip.filled-mid{background:var(--yellow)}
.diff-pip.filled-high{background:var(--orange)}
.diff-pip.filled-max{background:var(--red)}

/* ── Hazards ── */
.hazards-section{margin-bottom:12px}
.hazards-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}
@media(max-width:600px){.hazards-grid{grid-template-columns:repeat(2,1fr)}}
.hazard-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-md);padding:12px 14px;display:flex;align-items:center;gap:10px}
.hazard-icon{font-size:20px;flex-shrink:0}
.hazard-info{flex:1;min-width:0}
.hazard-name{font-size:11px;font-weight:700;color:var(--text);margin-bottom:4px}
.hazard-bar-track{width:100%;height:4px;background:var(--border);border-radius:2px;overflow:hidden}
.hazard-bar-fill{height:100%;border-radius:2px}
.hazard-level{font-size:9px;color:var(--text-faint);margin-top:3px}
.haz-1{background:var(--green)}.haz-2{background:color-mix(in oklch,var(--green) 60%,var(--yellow))}.haz-3{background:var(--yellow)}.haz-4{background:var(--orange)}.haz-5{background:var(--red)}

/* ── Language Requirements ── */
.lang-section{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:18px 22px;margin-bottom:12px}
.lang-list{display:flex;flex-direction:column;gap:10px}
.lang-row{display:flex;align-items:center;gap:12px}
.lang-name{font-size:13px;font-weight:700;color:#fff;min-width:90px}
.lang-level-pill{display:inline-flex;align-items:center;padding:3px 9px;border-radius:3px;font-size:10px;font-weight:700;letter-spacing:.05em;white-space:nowrap}
.llp-essential{background:color-mix(in oklch,var(--red) 14%,transparent);color:var(--red);border:1px solid color-mix(in oklch,var(--red) 28%,transparent)}
.llp-helpful{background:color-mix(in oklch,var(--yellow) 12%,transparent);color:var(--yellow);border:1px solid color-mix(in oklch,var(--yellow) 22%,transparent)}
.llp-optional{background:color-mix(in oklch,var(--green) 12%,transparent);color:var(--green);border:1px solid color-mix(in oklch,var(--green) 22%,transparent)}
.llp-senior{background:color-mix(in oklch,var(--accent) 12%,transparent);color:var(--accent);border:1px solid color-mix(in oklch,var(--accent) 22%,transparent)}
.lang-bar-wrap{flex:1;max-width:180px}
.lang-bar-track{width:100%;height:4px;background:var(--border);border-radius:2px;overflow:hidden}
.lang-bar-fill{height:100%;border-radius:2px;background:var(--accent)}

/* ── City Heatmap ── */
.heatmap-section{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:18px 22px;margin-bottom:12px}
.heatmap-header{display:flex;align-items:baseline;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:8px}
.heatmap-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(130px,1fr));gap:8px}
.city-card{border-radius:var(--radius-md);padding:12px 14px;position:relative;overflow:hidden;cursor:default;transition:transform .15s var(--ease),box-shadow .15s}
.city-card:hover{transform:translateY(-2px);box-shadow:0 6px 20px rgba(0,0,0,.4)}
.city-name{font-size:12px;font-weight:700;color:#fff;margin-bottom:2px}
.city-region{font-size:9px;color:rgba(255,255,255,.5);margin-bottom:6px;text-transform:uppercase;letter-spacing:.06em}
.city-salary{font-size:14px;font-weight:800;color:#fff;font-family:var(--font-display)}
.city-cost{font-size:9px;color:rgba(255,255,255,.55);margin-top:2px}
.heatmap-scale{display:flex;align-items:center;gap:8px;margin-top:10px;flex-wrap:wrap}
.heatmap-scale-bar{display:flex;height:6px;border-radius:3px;overflow:hidden;flex:1;max-width:200px}
.hs-low{background:var(--red);flex:1}.hs-mid{background:var(--yellow);flex:1}.hs-high{background:var(--green);flex:1}
.heatmap-scale-labels{display:flex;justify-content:space-between;width:100%;max-width:200px;font-size:9px;color:var(--text-faint)}

/* ── Requirement Extension ── */
.extended-section{margin-bottom:12px}
.extended-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:12px}
@media(max-width:768px){.extended-grid{grid-template-columns:repeat(2,1fr)}}
@media(max-width:480px){.extended-grid{grid-template-columns:1fr}}
.link-grid-extended{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}
@media(max-width:900px){.link-grid-extended{grid-template-columns:repeat(2,1fr)}}
@media(max-width:560px){.link-grid-extended{grid-template-columns:1fr}}
.link-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:14px 16px;transition:border-color .2s,transform .15s var(--ease)}
.link-card:hover{border-color:var(--border-hi);transform:translateY(-1px)}
.link-card-title{font-size:12px;font-weight:700;color:#fff;margin-bottom:4px}
.link-card-desc{font-size:10px;color:var(--text-muted);line-height:1.5}
.note-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:12px}
...
.note-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:16px 18px}
.note-title{font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--text-muted);margin-bottom:6px}
.note-value{font-family:var(--font-display);font-size:16px;font-weight:800;color:#fff;line-height:1.2;margin-bottom:6px}
.note-meta{font-size:11px;color:var(--text-muted);line-height:1.55}

/* ── Section divider ── */
.sec-divider{height:1px;background:var(--border);margin:20px 0}

/* ── Light / Dark Toggle ── */
.theme-toggle{display:inline-flex;align-items:center;gap:6px;padding:8px 14px;border-radius:var(--radius-md);border:1px solid var(--border-hi);background:var(--surface);color:var(--text-muted);font-size:12px;font-weight:700;cursor:pointer;transition:all .18s var(--ease);letter-spacing:.04em}
.theme-toggle:hover{border-color:var(--accent);color:var(--text)}
.theme-toggle .th-icon{font-size:15px;line-height:1}

.btn-ghost{display:inline-flex;align-items:center;gap:6px;padding:8px 14px;border-radius:var(--radius-md);border:1px solid var(--border-hi);background:var(--surface);color:var(--text-muted);font-size:12px;font-weight:700;cursor:pointer;transition:all .18s var(--ease);letter-spacing:.04em}
.btn-ghost:hover{border-color:var(--accent);color:var(--text)}
.btn-login{display:inline-flex;align-items:center;gap:7px;padding:9px 18px;border-radius:var(--radius-md);font-size:13px;font-weight:700;letter-spacing:.02em;background:var(--surface-3);color:#fff;border:1px solid var(--border-hi);transition:all .18s var(--ease);cursor:pointer}
.btn-login:hover{background:var(--surface-2);border-color:var(--text-muted)}

/* ── Light Mode Variables ── */
html[data-theme="light"]{
  --bg:#f4f4f5;--surface:#ffffff;--surface-2:#f0f0f0;--surface-3:#e8e8e8;
  --border:#e0e0e0;--border-hi:#cccccc;
  --text:#111111;--text-muted:#555555;--text-faint:#999999;
  --green:#16a34a;--yellow:#ca8a04;--orange:#ea580c;--red:#dc2626;
  --accent:#2a7b8a;--accent-hover:#1e6474;
}
html[data-theme="light"] body{color:var(--text)}
html[data-theme="light"] body::before{background:repeating-linear-gradient(0deg,transparent,transparent 2px,rgba(0,0,0,.02) 2px,rgba(0,0,0,.02) 4px)}
html[data-theme="light"] .page-title{color:#111}
html[data-theme="light"] .col-action{color:#111}
html[data-theme="light"] thead{background:var(--surface-2)}
html[data-theme="light"] .modal{background:var(--surface)}
html[data-theme="light"] .modal-title{color:#111}
html[data-theme="light"] .car-viz-val{color:#111}
html[data-theme="light"] .insight-value{color:#111}
html[data-theme="light"] .note-value{color:#111}
html[data-theme="light"] .ltv-val.cl-total{color:#111}
html[data-theme="light"] .city-name{color:#111}
html[data-theme="light"] .city-salary{color:#111}
html[data-theme="light"] .promo-label{color:var(--text-muted)}
html[data-theme="light"] .table-wrap{background:var(--surface)}
html[data-theme="light"] tbody tr:hover{background:var(--surface-2)}
html[data-theme="light"] .summary-card strong{color:#111}
html[data-theme="light"] .modal-input{background:var(--surface-2);color:#111}
html[data-theme="light"] .lang-name{color:#111}
</style>
</head>
<body>
...
<div class="page">
  ...
  <header class="header">
    <div>
      ...
      <h1 class="page-title">Job Automation<br>Compatibility List</h1>
  ...
  <div class="summary-row">
    <div class="summary-card"><strong>Survival Strategy</strong>Focus on Plating &amp; Tasting and Recipe Creation — the highest-value nodes least penetrable by AI. Build your chef brand and sharpen sensory judgment.</div>
...
// ── Country Data ──────────────────────────────────
const COUNTRIES = {
  US:{ name:'United States', symbol:'$', annualUSD:56000, annualLocal:56000, career:30, automationIdx:37,
       carName:'Toyota Camry', carPriceUSD:28000, riskAge:47, startAge:25,
       localDisplay:'$56,000/yr (USD)' },
  UK:{ name:'United Kingdom', symbol:'£', annualUSD:44000, annualLocal:35000, career:30, automationIdx:37,
       carName:'Ford Focus', carPriceUSD:26000, riskAge:49, startAge:25,
       localDisplay:'~£35K/yr (~$44K)' },
  AU:{ name:'Australia', symbol:'A$', annualUSD:42000, annualLocal:65000, career:30, automationIdx:37,
       carName:'Toyota RAV4', carPriceUSD:28000, riskAge:48, startAge:25,
       localDisplay:'~A$65K/yr (~$42K)' },
  DE:{ name:'Germany', symbol:'€', annualUSD:41000, annualLocal:38000, career:30, automationIdx:37,
       carName:'VW Golf', carPriceUSD:30000, riskAge:50, startAge:25,
       localDisplay:'~€38K/yr (~$41K)' },
  JP:{ name:'Japan', symbol:'¥', annualUSD:28000, annualLocal:4200000, career:30, automationIdx:37,
       carName:'Toyota Corolla', carPriceUSD:18000, riskAge:52, startAge:25,
       localDisplay:'~¥4.2M/yr (~$28K)' },
  SG:{ name:'Singapore', symbol:'S$', annualUSD:31000, annualLocal:42000, career:30, automationIdx:37,
       carName:'Toyota Corolla*', carPriceUSD:88000, riskAge:46, startAge:25,
       localDisplay:'~S$42K/yr (~$31K)' },
  TW:{ name:'Taiwan', symbol:'NT$', annualUSD:15000, annualLocal:480000, career:30, automationIdx:37,
       carName:'Toyota Corolla', carPriceUSD:22000, riskAge:45, startAge:25,
       localDisplay:'~NT$480K/yr (~$15K)' },
  CN:{ name:'China', symbol:'¥', annualUSD:12000, annualLocal:86000, career:30, automationIdx:37,
       carName:'BYD Seagull', carPriceUSD:12000, riskAge:44, startAge:25,
...
function buildCarGrid(safeCount, riskCount){
  ...
  for(let i=0;i<total;i++){
    ...
    svg.setAttribute('width','38');svg.setAttribute('height','22');
    ...
    svg.style.opacity='0';svg.style.transform='scale(.7)';
...
function renderCountry(code, animate=true){
  ...
  carSub.textContent = `Based on $${c.carPriceUSD.toLocaleString('en-US')} (USD)/unit reference price${sgNote} · 🟢 Safe earnings · 🔴 At-risk earnings`;
...
const obs = new IntersectionObserver(entries=>{
  if(entries[0].isIntersecting){ renderCountry('US',false); obs.disconnect(); }
},{threshold:0.1});
...
// USD equiv annual salary per country per level
const LEVEL_USD={
...
const CITY_DATA={
  US:[{city:'San Francisco',region:'West',salary:72000,cost:105},{city:'New York',region:'Northeast',salary:68000,cost:103},{city:'Boston',region:'Northeast',salary:64000,cost:90},{city:'Seattle',region:'West',salary:62000,cost:88},{city:'Los Angeles',region:'West',salary:65000,cost:95},{city:'Chicago',region:'Midwest',salary:58000,cost:80},{city:'Denver',region:'Mountain',salary:55000,cost:78},{city:'Miami',region:'South',salary:52000,cost:75},{city:'Dallas',region:'South',salary:50000,cost:65},{city:'Phoenix',region:'Southwest',salary:48000,cost:60}],
  UK:[{city:'London',region:'England',salary:55000,cost:100},{city:'Edinburgh',region:'Scotland',salary:44000,cost:75},{city:'Bristol',region:'England',salary:43000,cost:73},{city:'Manchester',region:'England',salary:42000,cost:72},{city:'Birmingham',region:'England',salary:40000,cost:70},{city:'Leeds',region:'England',salary:39000,cost:68}],
  AU:[{city:'Sydney',region:'NSW',salary:88000,cost:100},{city:'Melbourne',region:'VIC',salary:82000,cost:92},{city:'Canberra',region:'ACT',salary:80000,cost:85},{city:'Perth',region:'WA',salary:78000,cost:82},{city:'Brisbane',region:'QLD',salary:75000,cost:80},{city:'Gold Coast',region:'QLD',salary:70000,cost:75},{city:'Adelaide',region:'SA',salary:68000,cost:72}],
  DE:[{city:'Munich',region:'Bavaria',salary:72000,cost:100},{city:'Frankfurt',region:'Hesse',salary:68000,cost:95},{city:'Stuttgart',region:'Baden-W.',salary:66000,cost:88},{city:'Hamburg',region:'Hamburg',salary:65000,cost:88},{city:'Düsseldorf',region:'NRW',salary:60000,cost:82},{city:'Berlin',region:'Berlin',salary:60000,cost:80},{city:'Cologne',region:'NRW',salary:58000,cost:78}],
  JP:[{city:'Tokyo',region:'Kanto',salary:68000,cost:100},{city:'Yokohama',region:'Kanto',salary:62000,cost:90},{city:'Osaka',region:'Kansai',salary:58000,cost:85},{city:'Kyoto',region:'Kansai',salary:55000,cost:82},{city:'Nagoya',region:'Chubu',salary:52000,cost:75},{city:'Fukuoka',region:'Kyushu',salary:47000,cost:70},{city:'Sapporo',region:'Hokkaido',salary:45000,cost:68}],
  SG:[{city:'Orchard',region:'Central',salary:82000,cost:103},{city:'CBD / Marina',region:'Central',salary:80000,cost:100},{city:'Tampines',region:'East',salary:64000,cost:80},{city:'Jurong',region:'West',salary:62000,cost:78},{city:'Woodlands',region:'North',salary:60000,cost:72}],
  TW:[{city:'Taipei',region:'Northern',salary:30000,cost:100},{city:'Hsinchu',region:'Northern',salary:28000,cost:82},{city:'New Taipei',region:'Northern',salary:27000,cost:88},{city:'Taichung',region:'Central',salary:24000,cost:72},{city:'Kaohsiung',region:'Southern',salary:23000,cost:68},{city:'Tainan',region:'Southern',salary:22000,cost:65},{city:'Taitung',region:'Eastern',salary:18000,cost:55},{city:'Hualien',region:'Eastern',salary:17000,cost:52}],
  CN:[{city:'Shanghai',region:'East',salary:30000,cost:105},{city:'Beijing',region:'North',salary:28000,cost:100},{city:'Shenzhen',region:'South',salary:28000,cost:98},{city:'Guangzhou',region:'South',salary:24000,cost:85},{city:'Chengdu',region:'Southwest',salary:18000,cost:65},{city:'Chongqing',region:'Southwest',salary:17000,cost:60},{city:'Wuhan',region:'Central',salary:16000,cost:58},{city:"Xi'an",region:'Northwest',salary:14000,cost:52}]
};
const COUNTRY_META={
...
const EXTRA_META={
  US:{budget:'$450/mo',hnwi:'Private clubs · resort dining · estate chef ladder',startup:'68% readiness',mbti:'ESTP · ISTJ · ENTJ',uni:'Not required',uniMeta:'Prestige helps less than Michelin-grade references and consistency under service pressure.',privateSchool:'Useful but debt-sensitive',privateMeta:'Private culinary school helps with network and polish, but line experience still wins in many kitchens.',schools:'CIA · Johnson & Wales · Kendall',retire:'Best later-career exits: corporate dining leadership, instructor roles, private chef work, product R&D.',pivot:'Best pivot set: estate chef, kitchen ops consultant, content/demo chef.'},
  UK:{budget:'£260/mo',hnwi:'Hotels · members clubs · private events',startup:'54% readiness',mbti:'ISTJ · ESTJ · ENTJ',uni:'Optional',uniMeta:'Hospitality pedigree matters in some London groups, but references and practical output dominate.',privateSchool:'Helpful for placement',privateMeta:'Private schools can speed early placement, but apprenticeship kitchens still carry more weight long term.',schools:'Westminster Kingsway · Le Cordon Bleu London · University College Birmingham',retire:'Best later-career exits: hotel training, F&B management, consultancy, premium catering.',pivot:'Best pivot set: members-club chef, luxury catering lead, compliance trainer.'},
  AU:{budget:'A$420/mo',hnwi:'Resorts · wine regions · premium tourism',startup:'72% readiness',mbti:'ESTP · ENTJ · ISTJ',uni:'Optional',uniMeta:'TAFE and real kitchen output usually matter more than elite university branding.',privateSchool:'Often practical',privateMeta:'Private programs can be useful when tied to placement, but should not replace live service volume.',schools:'Le Cordon Bleu Melbourne · William Angliss · TAFE NSW Hospitality',retire:'Best later-career exits: catering operator, venue manager, hospitality trainer, food business owner.',pivot:'Best pivot set: private dining operator, venue consultant, regional culinary trainer.'},
  DE:{budget:'€220/mo',hnwi:'Luxury hotels · old-money dining · cruise / resort paths',startup:'49% readiness',mbti:'ISTJ · INTJ · ESTJ',uni:'Usually not required',uniMeta:'Formal training structure matters more than prestige university signaling.',privateSchool:'Apprenticeship usually stronger',privateMeta:'The apprenticeship route is often more trusted than expensive private alternatives.',schools:'Culinary schools tied to IHK tracks · Hotelfachschule Heidelberg · DHBW hospitality routes',retire:'Best later-career exits: training kitchens, hotel operations, procurement and food safety roles.',pivot:'Best pivot set: food safety lead, hotel trainer, procurement specialist.'},
  JP:{budget:'¥45,000/mo',hnwi:'Omakase · ryokan · private invitation dining',startup:'46% readiness',mbti:'ISTJ · INTJ · ISFJ',uni:'Not required',uniMeta:'Lineage, discipline, and master-apprentice credibility matter more than elite university names.',privateSchool:'Can help, lineage stronger',privateMeta:'School can open doors, but hierarchy and apprenticeship quality remain the stronger signal.',schools:'Tsuji Culinary Institute · Hattori Nutrition College · Tokyo Seika',retire:'Best later-career exits: teaching, quality control, supplier advisory, small-format premium dining. Includes potential for parental retirement support.',pivot:'Best pivot set: luxury counter chef, QA/standards lead, craft instructor.'},
  SG:{budget:'S$420/mo',hnwi:'Luxury hotels · expat clients · regional affluent events',startup:'76% readiness',mbti:'ENTJ · ESTJ · ESTP',uni:'Useful but optional',uniMeta:'Prestige helps in hotel groups, but multilingual polish and operational discipline matter more.',privateSchool:'Useful if placement-led',privateMeta:'Private training is most useful when tied to brand-name hotel placement and language advantage.',schools:'At-Sunrice · SHATEC · Temasek hospitality tracks',retire:'Best later-career exits: consulting, regional training, private dining, multi-outlet oversight. Includes potential for parental retirement support.',pivot:'Best pivot set: cloud-kitchen founder, private dining operator, regional trainer.'},
  TW:{budget:'NT$8,000/mo',hnwi:'Private banquets · boutique hospitality · luxury clubs',startup:'57% readiness',mbti:'ISTJ · ESTP · ENTJ',uni:'Optional',uniMeta:'Prestige matters less than bilingual service quality and steady kitchen reputation.',privateSchool:'Selective value',privateMeta:'Private school can help with early access, but consistent kitchen output remains the strongest signal.',schools:'國立高雄餐旅大學 · 景文科大餐飲系 · 實踐大學 hospitality tracks',retire:'Best later-career exits: banquet operations, culinary teaching, central kitchen roles. Includes potential for parental retirement support.',pivot:'Best pivot set: banquet lead, product testing, kitchen standards consultant.'},
  CN:{budget:'¥1,800/mo',hnwi:'Luxury banquets · private rooms · club kitchens',startup:'61% readiness',mbti:'ISTJ · ESTP · ENTJ',uni:'Not required',uniMeta:'Brand-name university is much less important than apprenticeship pedigree, execution, and network.',privateSchool:'Mixed value',privateMeta:'Private culinary schools can speed placement, but live kitchen references usually outrank tuition branding.',schools:'Le Cordon Bleu Shanghai · SICA · Shanghai Business & Tourism',retire:'Best later-career exits: training manager, central kitchen leader, private chef, culinary lecturer. Includes potential for parental retirement support.',pivot:'Best pivot set: private household chef, SOP consultant, flavor/product R&D.'}
};
const ACCESS_LINKS=[
...
const COUNTRY_NAMES={US:'USA',UK:'United Kingdom',AU:'Australia',DE:'Germany',JP:'Japan',SG:'Singapore',TW:'Taiwan',CN:'China'};
...
function renderIncomeChart(country){
  ...
  const TOTAL_YRS=30;
  const W=900,H=240,padL=55,padR=30,padT=24,padB=38;
  ...
  for(let g=0;g<=4;g++){
    ...
    html+=`<text x="${padL-5}" y="${y+4}" text-anchor="end" fill="#555" font-size="10">${val>=1000?'$'+(val/1000).toFixed(0)+'K':val}</text>`;
  }
  // X axis labels (0 to 30)
  [0,5,10,15,20,25,30].forEach(yr=>{
    const lbl=`Yr ${yr}`;
    html+=`<text x="${xOf(yr)}" y="${H-5}" text-anchor="middle" fill="#555" font-size="10">${lbl}</text>`;
  });

  // Career end divider at yr 30
  html+=`<line x1="${xOf(30)}" y1="${padT}" x2="${xOf(30)}" y2="${H-padB}" stroke="#555" stroke-width="1.5" stroke-dasharray="4,3"/>`;
  html+=`<text x="${xOf(30)+4}" y="${padT+12}" fill="#666" font-size="9" font-weight="600">CAREER END</text>`;
  ...
  // Dots at career transitions
  const transYears=[0,2,5,8,16,30];
  transYears.forEach(yr=>{
    const p=pts.find(pp=>pp.yr===yr);
    if(!p)return;
    const col=LEVELS[p.lvl].color;
    html+=`<circle cx="${xOf(p.yr)}" cy="${yOf(p.sal)}" r="5" fill="${col}" stroke="#0e0e0e" stroke-width="2"/>`;
    html+=`<line x1="${xOf(p.yr)}" y1="${yOf(p.sal)}" x2="${xOf(p.yr)}" y2="${H-padB+4}" stroke="${col}" stroke-width="1" stroke-dasharray="3,3" opacity="0.35"/>`;
  });

  // Current level band highlight
  ...
  const rx1=xOf(lvlStart),rx2=xOf(lvlEnd);
  html=`<defs><linearGradient id="careerGrad" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stop-color="#4f98a3"/><stop offset="60%" stop-color="#eab308"/><stop offset="100%" stop-color="#22c55e"/></linearGradient></defs><rect x="${rx1}" y="${padT}" width="${rx2-rx1}" height="${cH}" fill="${lvlColor}" opacity="0.07" rx="2"/>`+html;
...
function renderInsights(country){
  ...
  const difLabel=['','','Very Easy','Easy','Below Avg','Average','Above Avg','Hard','Very Hard','Brutal','Extreme'][m.difficulty]||'Hard';
  grid.innerHTML=`
...
function renderLang(country){
  ...
  const levelPillClass={
    'Essential':'llp-essential','Helpful':'llp-helpful','Optional':'llp-optional',
  ...
  list.innerHTML=langs.map(l=>`
    <div class="lang-row">
      ...
      <div style="font-size:10px;color:var(--text-faint);min-width:30px">${l.score}/5</div>
    </div>
  `).join('');
}
...
function renderHeatmap(country){
  ...
  if(!grid)return;
  if(label)label.textContent=COUNTRY_NAMES[country]||country;
  if(!cities.length){grid.innerHTML='<div style="font-size:12px;color:var(--text-faint)">No city data available</div>';return;}
  const sorted=[...cities].sort((a,b)=>b.salary-a.salary);
  const maxS=sorted[0].salary,minS=sorted[sorted.length-1].salary;
  grid.innerHTML=sorted.map(c=>{
    ...
    if(pct<0.5){
      const t=pct*2;
      const r=Math.round(239+(234-239)*t),g=Math.round(68+(179-68)*t),b=Math.round(68+(8-68)*t);
      bg=`rgba(${r},${g},${b},0.25)`;
    } else {
      const t=(pct-0.5)*2;
  ...
  if(network){
    network.innerHTML=`
      ...
      <div class="note-card" style="grid-column:span 2">
        <div class="note-title">Hall of Fame</div>
  ...
  if(extra){
    extra.innerHTML=`
      ...
      <div class="insight-card">
        ...
        <div class="insight-value" style="color:var(--green)">${meta.startup}</div>
  ...
  if(notes){
    notes.innerHTML=`
      ...
      <div class="note-card">
        ...
        <div class="note-meta">Your late-career exit options and personal retirement outlook. Parental support potential is shown where culturally applicable.</div>
      </div>
      <div class="note-card">
        <div class="note-title">Skill Pivot</div>
        <div class="note-value" style="font-size:13px">${meta.pivot.toUpperCase()}</div>
...
function renderLevelLTV(level, country){
  ...
  document.getElementById('car-safe-label').textContent=`${safeCarCount} cars = Human-Safe LTV (${c.symbol}${safe.toLocaleString('en-US')})`;
  document.getElementById('car-risk-label').textContent=`${riskCarCount} cars = At-Risk LTV (${c.symbol}${risk.toLocaleString('en-US')})`;
...
function updateLevelPcts(country){
  ...
  LEVELS.forEach((_,i)=>{
    ...
    if(!el)return;
    if(i===LEVELS.length-1){el.textContent='Peak';return;}
    const pct=Math.round(salaries[i]/maxSal*100);
    el.textContent=pct+'% of peak';
  });
}
